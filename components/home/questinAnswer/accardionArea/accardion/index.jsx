...
const Accardion = ({ i, expanded, setExpanded, title, content }) => {
  ...
  return (
    ...
    >
      <div className="flex w-full px-3 cursor-pointer justify-between gap-x-10 bg-white rounded-xl h-[54px] items-center text-midnight-950 font-medium text-sm md:text-lg">
        {title}
        <ButtonComponent
          initial={false}
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          <MotionDiv
            ...
            animate="open"
            exit="collapsed"
            className="max-h-[60px] cursor-pointer md:max-h-[120px] overflow-y-auto scrollbar font-normal text-[10px] md:text-[18px] leading-4 md:leading-6 font-Inter"
            variants={{
